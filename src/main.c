...
Font score_font;
Vector2 selected_tile = { -1, -1 }; // to track selected tile position
...
{
    char temp_board = board[y1][x1];
    board[y1][x1] = board[y2][x2];
    board[y2][x2] = temp_board;
}
...
void add_score_popup(int x, int y, int amount, Vector2 grid_origin)
{
    for(int i = 0; i < MAX_SCORE_POPUPS; i++)
    {
        ...
        {
            score_popups[i].pos = (Vector2){ grid_origin.x + x * TILE_SIZE + TILE_SIZE / 2,
...
int main(void)
{
    ...
    InitWindow(screen_width, screen_height, "2D ASCII MATCH3");
    SetTargetFPS(60);
    srand(time(NULL));
    ...
    background_music = LoadMusicStream("assets/Pixel Dreams.mp3");
    match_sound = LoadSound("assets/matched.mp3");
    ...
    while(!WindowShouldClose())
    {
        ...
        if(tile_state == STATE_IDLE && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            if(x >= 0 && x < BOARD_SIZE && y >= 0 && y < BOARD_SIZE)
            {
                ...
                {
                    selected_tile.x = current_tile.x;
                    selected_tile.y = current_tile.y;
                } else if(selected_tile.x == current_tile.x && selected_tile.y == current_tile.y)
                {
                    selected_tile.x = -1;
                    selected_tile.y = -1;
                } else if(are_tiles_adjacent(selected_tile, current_tile))
                {
            ...
            for(int x = 0; x < BOARD_SIZE; x++)
            {
                ...
                if(board[y][x] != ' ')
                {
                    DrawTextEx(
                        ...
                        (Vector2){ rect.x + 15, rect.y + 10 - fall_offset[y][x]},
                        20, 1,
        ...
        for(int i = 0; i < MAX_SCORE_POPUPS; i++)
        {
            if(score_popups[i].active)
            {
                ...
                DrawText(TextFormat("+%d", score_popups[i].amount), score_popups[i].pos.x, score_popups[i].pos.y, 20, c);
            }
        }
    ...
    UnloadTexture(background); // unload resources
